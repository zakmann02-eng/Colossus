...
Pre-filters (all must pass):
  ...
  - Resolution time must be known and within 7 days (live or this week only)
...
MAX_DAYS_OUT = 45 * 86_400  # up to 45 days: covers MLB/MLS playoff-winner markets
...
async def evaluate_market(market: dict, client: "PolymarketClient") -> TradeSignal | None:
    ...
    if secs > MAX_DAYS_OUT:
        _skip("far-future")
        logger.info("SKIP far-future(%.1fd): slug=%s q=%s", secs / 86400, market_slug[:30], question[:60])
        return None
